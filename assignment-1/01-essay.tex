...
\begin{quote}
    ...
    We usually consider functions for which sets $A$ and $B$ are sets of real numbers. 
    The symbol $f(x)$ is read as "$f$ of $x$" or "$f$ at $x$" and is called the value of $f$ at $x$, or the image of $x$ under $f$. 
    Set $A$ is called the domain of the function. 
    The range of $f$ is the set of all possible values of $f(x)$ as $x$ varies over the entire domain. (p. 125)
